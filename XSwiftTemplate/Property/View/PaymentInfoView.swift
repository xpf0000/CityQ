...
class PaymentInfoView: UIView {
    ...
    func show()
    {
        ...
        self.table.httpHandle.url = "http://101.201.169.38/api/Public/Found/?service=Wuye.getPayList&uid=\(DataCache.Share().userModel.uid)&username=\(DataCache.Share().userModel.username)&fanghaoid="+DataCache.Share().userModel.house.fanghaoid+"&type=\(type)&status=\(status)"
